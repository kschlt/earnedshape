...
v0.2.1 applies the listed accepted change proposals to the protocol text. It adds no proposal of its own; each change traces to one proposal, and [`docs/review/disposition.md`](docs/review/disposition.md) records where each landed. File names are unchanged; headings and status lines read v0.2.1.
...
| CP-04 | `01` §3: Progressive Commitment is the one tentativeness ladder, with one presentation rule; §6 becomes guidance; two dimensions of `02` §7 are its state encoding | finding F-07: four overlapping tentativeness devices | — |
...
| CP-18 | `01` §9: record what is discarded, and the cost of reconstructing it, before any irreversible normalization; `CAP-16`; extended event kind `normalization.recorded` | Case 01: the terminology rule's general form | whether recorded losses are ever needed again (`CAP-16`) |
...
| CP-21 | `01` §12: counterexamples classified local or structural; counterexample work saturated when new ones cause mostly local changes | Case 01's counterexample method, dropped in v0.2 | whether saturation predicts a stable model |
